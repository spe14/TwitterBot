...
public class TweetParser {
    ...
    static List<String> csvFileToTweets(String pathToCSVFile, int tweetColumn) {
        ...
        try { 
            while (f.hasNext()) {
                String csvLine = f.next();
                String t = extractColumn(csvLine, tweetColumn);
                if (t != null) {
                    tweets.add(t);
                }
            }
    ...
    static List<List<String>> parseAndCleanTweet(String tweet) {
        ...
        List<String> s = sentenceSplit(noURLs);
        ...
        for (String a : s) {
            cleaned.add(parseAndCleanSentence(a));
        }
        return cleaned;
    }
    ...
    public static List<List<String>> csvFileToTrainingData(String pathToCSVFile, int tweetColumn) {
        ...
        List<List<String>> cleanedTweets = new LinkedList<List<String>>();
        try {
            for (String s : tweets) {
                cleanedTweets.addAll(parseAndCleanTweet(s));
            }
        } catch (IllegalArgumentException e) {
