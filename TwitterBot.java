...
public class TwitterBot {
    ...
    public TwitterBot(String csvFile, int tweetColumn, NumberGenerator ng) {
        ...
        this.ng = ng;
        List<List<String>> cleanedTweets = 
                TweetParser.csvFileToTrainingData(csvFile, tweetColumn);
        
        for (int i = 0; i < cleanedTweets.size(); i++) {
            List<String> m = cleanedTweets.get(i);
            mc.train(m.iterator());
        }
    }
